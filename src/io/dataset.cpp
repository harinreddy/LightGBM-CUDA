...
void Dataset::Construct(
  ...
  // ibmGBT updated "gpu" flag to "cuda"
  ...
  if (io_config.enable_bundle && !used_features.empty()) {
    features_in_group = FastFeatureBundling(bin_mappers,
                                            ...
                                            sparse_threshold_, io_config.is_enable_sparse, io_config.device_type == std::string("cuda"));
  }
...
// ibmGBT: return true if resized (else: false)
...
void Dataset::CopySubset(const Dataset* fullset, const data_size_t* used_indices, data_size_t num_used_indices, bool need_meta_data) {
  ...
  OMP_INIT_EX();
  //#pragma omp parallel for schedule(static) // ibmGBT: move parallelism to bin
  for (int group = 0; group < num_groups_; ++group) {
    OMP_LOOP_EX_BEGIN();
