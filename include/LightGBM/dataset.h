...
class Dataset {
 public:
  ...
  bool ReSize(data_size_t num_data); // ibmGBT: return true if resized (else: false)  
  ...
  */
...
#if 0
  // ibmGBT
  inline void FeatureGroupPinData(int group) const {
    feature_groups_[group]->FeatureGroupPinData();
  }

  // ibmGBT
  inline void FeatureGroupPinData() const {
    OMP_INIT_EX();
    #pragma omp parallel for schedule(guided)
    for (int i = 0; i < num_groups_; ++i) {
      OMP_LOOP_EX_BEGIN(); 
      feature_groups_[i]->FeatureGroupPinData();
      OMP_LOOP_EX_END();
    }
    OMP_THROW_EX();
  }
  ...
  // ibmGBT
  inline void FeatureGroupUnPinData(int group) const {
    feature_groups_[group]->FeatureGroupUnPinData();
  }

  // ibmGBT
  inline void FeatureGroupUnPinData() const {
    OMP_INIT_EX();
    #pragma omp parallel for schedule(guided)
    for (int i = 0; i < num_groups_; ++i) {
      OMP_LOOP_EX_BEGIN();
      feature_groups_[i]->FeatureGroupUnPinData();
      OMP_LOOP_EX_END();
    }
    OMP_THROW_EX();
  }
