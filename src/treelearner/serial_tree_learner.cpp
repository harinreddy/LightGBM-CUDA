...
#ifdef TIMETAG
//std::chrono::duration<double, std::milli> init_train_time; // ibmGBT
//std::chrono::duration<double, std::milli> init_split_time; // ibmGBT
//std::chrono::duration<double, std::milli> hist_time; // ibmGBT
//std::chrono::duration<double, std::milli> find_split_time; // ibmGBT
//std::chrono::duration<double, std::milli> split_time; //ibmGBT
...
SerialTreeLearner::~SerialTreeLearner() {
  #ifdef TIMETAG
  Log::Info("SerialTreeLearner::init_train costs %f", init_train_time_ * 1e-3); // ibmGBT
  Log::Info("SerialTreeLearner::init_split costs %f", init_split_time_ * 1e-3); // ibmGBT
  Log::Info("SerialTreeLearner::hist_build costs %f", hist_time_ * 1e-3); // ibmGBT
  Log::Info("SerialTreeLearner::find_split costs %f", find_split_time_ * 1e-3); // ibmGBT
  Log::Info("SerialTreeLearner::split costs %f", split_time_ * 1e-3); // ibmGBT
...
void SerialTreeLearner::Init(const Dataset* train_data, bool is_constant_hessian, bool is_use_subset) {
  ...
  Log::Info("Number of fetched rows from the training set: %d \n", num_data_); 
  ...
  bool is_cuda = config_->device_type == std::string("cuda");

  data_partition_.reset(new DataPartition(num_data_, config_->num_leaves, is_cuda));
...
void SerialTreeLearner::BeforeTrain() {
  ...
  if (data_partition_->leaf_count(0) == num_data_) {
    // use all data
    //Log::Info("[1] Use all data: data_partition_->leaf_count(0): %d, num_data_: %d\n", data_partition_->leaf_count(0), num_data_); // ibmGBT
    smaller_leaf_splits_->Init(gradients_, hessians_);

  } else {
    //Log::Info("[2] Not use all data: data_partition_->leaf_count(0): %d, num_data_: %d\n", data_partition_->leaf_count(0), num_data_); // ibmGBT
...
void SerialTreeLearner::ConstructHistograms(const std::vector<int8_t>& is_feature_used, bool use_subtract) {
  ...
  #ifdef TIMETAG
  hist_time_ += std::chrono::steady_clock::now() - start_time; // ibmGBT
  #endif
}
