...
__global__ void KERNEL_NAME(const uchar* feature_data_base, 
     ...
     // assume this starts at 32 * 4 = 128-byte boundary // ibmGBT: What does it mean? boundary??
     ...
     //if(subglobal_tid==0) printf("feature_id: %d, feature_mask: %d\n", feature_id, feature_mask);
     if (!feature_mask) {
         return;
     } else {
         feature_mask = feature_mask - 1; // ibmGBT: feature_mask is used for get feature (1: 4bit feature, 0: 8bit feature)
     }
     //if(subglobal_tid==0) printf("feature_id: %d, feature_mask: %d\n", feature_id, feature_mask);
     ...
     gh_hist[ltid * 2 + 1] = const_hessian * cnt_hist[ltid]; // ibmGBT: counter move to this position 
     __syncthreads();
     #endif
     ...
     if (*counter_val == (1 << power_feature_workgroups) - 1) {
         if (ltid == 0) {
             ...
             //if(feature_id == 0) printf("feature_id: %d, reduction group_id: %d\n", feature_id, group_id);
             //if(feature_id == 2) printf("feature_id: %d, reduction group_id: %d\n", feature_id, group_id);
         }
     //}
 #else
     }
