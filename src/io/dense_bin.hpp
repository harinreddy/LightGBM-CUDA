...
class DenseBin: public Bin {
 public:
  ...
  DenseBin(data_size_t num_data)
    : num_data_(num_data), data_(num_data_, static_cast<VAL_T>(0)), len_pin_(0) { // ibmGBT
  }
...
#if 0
      bool pin_data = false; // ibmGBT: un-pin data before resizing if it is pinned
      if (len_pin_ > 0) {
        UnPinData();
        pin_data = true;
      }
...
#if 0
      if (pin_data) PinData(); // ibmGBT: pin data if it was pinned
#endif
...
#if 0
  // ibmGBT
  void PinData() override {
     size_t len_pin_ = SizesInByte();

     if (data_.data() != NULL){
        cudaPointerAttributes attributes;
        cudaPointerGetAttributes (&attributes, data_.data());

        if ((attributes.type == cudaMemoryTypeHost) && (attributes.devicePointer != NULL)){
           CUDASUCCESS_OR_FATAL(cudaHostRegister(data_.data(), len_pin_, cudaHostRegisterDefault));
            printf("Pinned data of DenseBin (size: %lu)\n", len_pin_);
        }
     }
  }
 
  // ibmGBT
  void UnPinData() override {
    if (len_pin_ > 0) {
     ...
     if ((attributes.type == cudaMemoryTypeHost) && (attributes.devicePointer != NULL)){
        cudaError_t err = cudaHostUnregister(data_.data());
        if (err != cudaErrorHostMemoryNotRegistered) {
          CUDASUCCESS_OR_FATAL(err);
        }
        cudaGetLastError();
        len_pin_ = 0;
      }
  ...
  void CopySubset(const Bin* full_bin, const data_size_t* used_indices, data_size_t num_used_indices) override {
    ...
    #pragma omp parallel for schedule(static) // ibmGBT - NO performance gain..
...
//  std::vector<VAL_T> data_;
  ...
  size_t len_pin_; // ibmGBT

  DenseBin<VAL_T>(const DenseBin<VAL_T>& other)
    : num_data_(other.num_data_), data_(other.data_), len_pin_(other.len_pin_){} // ibmGBT
};
