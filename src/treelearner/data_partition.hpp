...
class DataPartition {
 public:
  DataPartition(data_size_t num_data, int num_leaves, bool is_cuda) // ibmGBT
    :num_data_(num_data), num_leaves_(num_leaves), is_cuda_(is_cuda) { // ibmGBT
 ...
 private:
  ...
  std::vector<data_size_t, CHAllocator<data_size_t>> indices_;
