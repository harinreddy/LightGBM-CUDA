...
std::chrono::duration<double, std::milli> total_train_time; // ibmGBT
std::chrono::duration<double, std::milli> bagging_gh_time; //ibmGBT
#endif // TIMETAG
...
balanced_bagging_(false) {
  ...
  #pragma omp master
  {
   ...
   // lightGBM_CUDA COMMENTED OUT 
   // lightGBM_CUDA setting the openMP threads to 160
    num_threads_ = 160;
  }
...
GBDT::~GBDT() {
  if (config_ != nullptr) {
  #ifdef TIMETAG
  Log::Info("GBDT::total train time %f", total_train_time * 1e-3); // ibmGBT
  ...
  Log::Info("GBDT::copy subset costs %f", copy_subset_time * 1e-3);
  Log::Info("GBDT::bagging costs %f", bagging_time * 1e-3);
  Log::Info("GBDT::tree costs %f", tree_time * 1e-3);
  Log::Info("GBDT::bagging gb costs %f", bagging_gh_time * 1e-3); // ibmGBT
  // ibmGBT
  if ( config_->log_train_time ){
    std::string log_filepath = config_->log_folder + "ibmGBT_gbdt_time.txt";
    std::ofstream resultfile(log_filepath.c_str(), std::ofstream::app);
    tree_learner_->GetHistTime();
    resultfile << config_->device_type << "	";
    resultfile << config_->objective << "	";
    resultfile << total_train_time.count() * 1e-3 << "	";
    resultfile << boosting_time.count() * 1e-3 << "	";
    resultfile << bagging_time.count() * 1e-3 << "	";
    resultfile << copy_subset_time.count() * 1e-3 << "	";
    resultfile << tree_time.count() * 1e-3 << "	";
    resultfile << bagging_gh_time.count() * 1e-3 << "	";
    resultfile << tree_learner_->GetInitTrainTime().count() * 1e-3 << "	";
    resultfile << tree_learner_->GetInitSplitTime().count() * 1e-3 << "	";
    resultfile << tree_learner_->GetHistTime().count() * 1e-3 << "	";
    resultfile << tree_learner_->GetFindSplitTime().count() * 1e-3 << "	";
    resultfile << tree_learner_->GetSplitTime().count() * 1e-3 << "\n";
    resultfile.close();
  }
  #endif
 }
...
void GBDT::Init(const Config* config, const Dataset* train_data, const ObjectiveFunction* objective_function,
 ...
 // ibmGBT: move tree_learner building position
  ...
  // ibmGBT: init tree learner with is_use_subset_ for CUDA
  // do not copy feature is is_use_subset for initialization
  // initialize training data info with bagging data size (tmp_subset_)
  ...
  // initialize tree learner with train_data_
  // for cuda, do the pre-processing only if it use bagging 
  // do GPU allocation and memory copy if it does not use bagging
  // tree_learner_->Init(train_data_, is_constant_hessian_, is_use_subset_);
...
void GBDT::Bagging(int iter) {
  ...
  if ((bag_data_cnt_ < num_data_ && iter % config_->bagging_freq == 0)
    ...
    if (!is_use_subset_) {
      ...
      // ibmGBT: pin data of subset
      if (resized & (config_->device_type == std::string("cuda"))) {

        // tmp_subset_->FeatureGroupPinData();
...
void GBDT::Train(int snapshot_freq, const std::string& model_output_path) {
  ...
  for (int iter = 0; iter < config_->num_iterations && !is_finished; ++iter) {
    ...
    total_train_time = end_time - start_time; // ibmGBT
    Log::Info("%f seconds elapsed, finished iteration %d", total_train_time * 1e-3, iter + 1); // ibmGBT
    if (snapshot_freq > 0
...
bool GBDT::TrainOneIter(const score_t* gradients, const score_t* hessians) {
 ...
 if ((config_->device_type == std::string("cuda")) && (iter_ == 0)) {
    ...
    Bagging(0); // ibmGBT: for overlapping bagging and tree computation
  ...
  for (int cur_tree_id = 0; cur_tree_id < num_tree_per_iteration_; ++cur_tree_id) {
    ...
    if (class_need_train_[cur_tree_id] && train_data_->num_features() > 0) {
      ...
      if (is_use_subset_ && bag_data_cnt_ < num_data_) {
        #pragma omp parallel for schedule(static) // ibmGBT: we have to make new method to make it work. The for loop should be ordered!
        for (int i = 0; i < bag_data_cnt_; ++i) {
          tmp_grad[i] = grad[bag_data_indices_[i]]; // ibmGBT
          tmp_hess[i] = hess[bag_data_indices_[i]]; // ibmGBT
      ...
      new_tree.reset(tree_learner_->Train(tmp_grad, tmp_hess, is_constant_hessian_, forced_splits_json_)); // ibmGBT
    }
  ...
  // ibmGBT: moved for overlapping data copy w/ other operations
...
std::string GBDT::OutputMetric(int iter) {
  ...
  // ibmGBT
  std::string log_filepath = config_->log_folder + "ibmGBT_acc.txt";

  // ibmGBT
  if ( config_->log_final_OutputMetric && iter == config_->num_iterations ) {
    std::ofstream resultfile(log_filepath.c_str(), std::ofstream::app);
    ...
    else Log::Fatal("Fail to open result file.\n");
  }
